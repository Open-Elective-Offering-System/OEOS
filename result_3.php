	...
	<div>
	 <ul id="rcorners">
      ...
      <?php
        include('config.php');
        $sub= $_POST['sub'];
        $sql = "SELECT DISTINCT * FROM DEPARTMENT AS D JOIN STAFF AS ST ON ST.DEPT_ID = D.DEPT_ID JOIN SUBJECT AS S ON S.SUBJECT_ID = ST.SUBJECT_ID JOIN RESULT AS R ON R.SUBJECT_ID = S.SUBJECT_ID JOIN REG_STUDENTS AS RS ON RS.ENROLL_ID = R.ENROLL_ID WHERE S.SUBJECT_ID='$sub'";
